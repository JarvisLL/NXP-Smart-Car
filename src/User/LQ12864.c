...
#define LCD_SCL_L   (GPIO_Ctrl( PORTC,16,0))
#define LCD_SCL_H   (GPIO_Ctrl( PORTC,16,1))

#define LCD_SDA_L   (GPIO_Ctrl(PORTC,17,0))
#define LCD_SDA_H   (GPIO_Ctrl(PORTC,17,1))

#define LCD_RST_L   (GPIO_Ctrl(PORTC,18,0))
#define LCD_RST_H   (GPIO_Ctrl(PORTC,18,1))

#define LCD_DC_L    (GPIO_Ctrl(PORTC,19,0))
#define LCD_DC_H    (GPIO_Ctrl(PORTC,19,1))
...
/**************** 成电疾影  ****************************
 *  * 函数名称 ：LCD_Write_DC
 *  * 函数参数 : unsigned char data,unsigned char dc
 *  * 返回值   : 无
 *  * 函数功能 ：写数据or命令,dc=1是写数据，dc=0写命令
 *  * 作    者 ：Joseph_Chen
 *  * 日    期 : 2015/06/05
********************************************************/
void LCD_Write_DC(unsigned char data,unsigned char dc)
{
  uint8 i = 0;
...
/**************** 成电疾影  ****************************
 *  * 函数名称 ：LCD_Set_XY
 *  * 函数参数 : unsigned char x,unsigned char y
 *  * 返回值   : 无
 *  * 函数功能 ：设置坐标
 *  * 作    者 ：Joseph_Chen
 *  * 日    期 : 2015/06/05
...
{
  //command
  LCD_Write_DC(0xb0+y,0); 
  LCD_Write_DC( ((x & 0xf0) >> 4) | 0x10,0);
  LCD_Write_DC( ( x & 0x0f) | 0x01,0);
}

/***********************************************************
 *  * 函数名称  ：INV_LCD_P8x16Str
 *  * 参    数  ：
 *  * 功    能  : 反色显示一组8*16标准ASCII字符串,坐标(x,y)，y为
                  页范围，0-7
 *	* 日		期	：2015/02/28
 *	*	作		者	：
************************************************************/
 void INV_LCD_P8x16Str(unsigned char x,unsigned char y,char *ch)
{
	...
	{    
		...
		LCD_Set_XY(x,y);    
		for ( i = 0 ; i < 8 ; i++ )     
			LCD_Write_DC(~F8X16[c * 16 + i],1);
		LCD_Set_XY(x,y + 1);    
		for ( i = 0 ; i < 8 ; i ++)     
			LCD_Write_DC(~F8X16[c * 16 + i + 8],1);  
		x += 8;
		j ++;
	}
}
  ...
  void LCD_Write_double2_2(unsigned char x,unsigned char y,double s)
  {
     ...
     LCD_Write_Char((num/1000)+0x30);//十位
     num = num % 1000;
     LCD_Write_Char((num/100)+0x30);//个位
     num = num % 100;
     LCD_Write_Char('.');
     LCD_Write_Char((num/10)+0x30);//小数点后一位
     num = num % 10;
     LCD_Write_Char((num)+0x30);//小数点后第二位  
  }
...
void LCD_Write_double1_3(unsigned char x,unsigned char y,double s)
  {
     ...
     LCD_Write_Char((num/1000)+0x30);//个位
     num = num % 1000;
	   LCD_Write_Char('.');			//写小数点
     LCD_Write_Char((num/100)+0x30);//小数点后一位
     num = num % 100;
     LCD_Write_Char((num/10)+0x30);//小数点后两位
     num = num % 10;
     LCD_Write_Char((num)+0x30);//小数点后第三位  
  }


/***********************************************************
 *  * 函数名称  ：LCD_Write_3_Num
 *  * 参    数  ：
 *  * 功    能  : 
 *	* 日		期	：2015/02/28
 *	*	作		者	：
